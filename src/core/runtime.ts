...
export class AgentRuntime implements IAgentRuntime {
  ...
  constructor(opts: {
    ...
    if (!this.getSetting("REMOVED") && !this.llamaService) {
  ...
  async completion({
    ...
    max_context_length = this.getSetting("REMOVED") ? 127000 : 8000,
    max_response_length = this.getSetting("REMOVED") ? 8192 : 4096,
  }): Promise<string> {
